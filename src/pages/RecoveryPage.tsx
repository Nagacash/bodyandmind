...
import recoveryImg from '../assets/images/recovery_flow_lounge_1785311965550.jpg';
    ...
    <div className="space-y-28 pt-28 pb-20 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 font-body">
        ...
        <section className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {RECOVERY_MODULES.map((mod) => (
              <div
                key={mod.id}
                className="p-8 glass-panel hover:border-[#8E7B62]/30 transition-all space-y-6 relative overflow-hidden flex flex-col justify-between"
              >
                {mod.isNew && (
                  <div className="absolute top-4 right-4 bg-[#8E7B62] text-[#0F0F0F] text-[10px] font-bold uppercase tracking-widest px-3 py-1 flex items-center gap-1">
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-xs font-semibold text-[#8E7B62]">{mod.tagline}</div>
                  <p className="text-xs text-muted leading-relaxed">{mod.description}</p>

                  <div className="pt-3 space-y-1.5">
                    {mod.benefits.map((b, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs text-muted">
                        <Check className="w-3.5 h-3.5 text-[#8E7B62] shrink-0" />
                        <span>{b}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="pt-4 border-t border-white/10">
                  <button
                    ...
                    className="w-full bg-[#0F0F0F] border border-[#222222] hover:bg-[#8E7B62] text-gray-200 hover:text-[#0F0F0F] py-3 rounded-none text-xs font-bold uppercase tracking-wider transition-all cursor-pointer active:scale-[0.98]"
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="timeline-step p-6 bg-[#0F0F0F]/80 border border-[#222222] space-y-3">
                <div className="font-mono text-xs font-bold text-[#8E7B62]">{s.step}</div>
      ...
      <RevealOnScroll>
        <GlassPanel className="p-8 space-y-4">
          <div className="flex items-center gap-2 text-[#8E7B62]">
